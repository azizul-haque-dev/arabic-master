...
export function ArabicTextsPage() {
  ...
  const [audioTarget, setAudioTarget] = useState<ArabicTextEntry | null>(null);
                ...
                <TableBody>
                  {data.items.map((entry) => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell className="max-w-xs text-muted truncate ">
                        {entry.meaningEn || "—"}
                      </TableCell>
                      <TableCell>
                        {entry.word ? (
      ...
      {audioDialogOpen && (
        <AudioUploadDialog
          ...
          onOpenChange={(open) => {
            ...
            if (!open) setAudioTarget(null);
          }}
          textId={audioTarget?.id ?? ""}
          existingAudioUrl={audioTarget?.audioUrl}
          queryKeyToInvalidate={["arabic-texts"]}
          title="Upload Audio"
          description={`Upload a pronunciation clip for "${audioTarget?.text ?? "this entry"}"`}
          triggerLabel={undefined} // Don't show trigger button, we trigger from dropdown
        />
      )}
    </>
  );
